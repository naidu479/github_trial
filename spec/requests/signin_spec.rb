...
describe "Signin API" do
  ...
  context "when the user is not present" do
    it "returns user not present" do
      headers = { "CONTENT_TYPE" => "application/json" }
      post "/login", '{ "user": { "email": "klo2132182@llop11.com", "password": "12345678" } }', headers
  ...
  context 'login with username' do
    ...
    it "returns user not present" do
      headers = { "CONTENT_TYPE" => "application/json" }
      post "/login", '{ "user": { "username": "klo2132182", "password": "12345678" } }', headers
...
describe "when the user logs out" do
  ...
  context "when the Auth Token is correct" do
    it "deletes the Auth Token from the DB" do
      headers = { "CONTENT_TYPE" => "application/json" }
      delete "/logout", '{"auth_token":"'+user.authtokens.last.token+'"}', headers
  ...
  context "when the Auth Token is incorrect" do
    it "returns 404" do
      headers = { "CONTENT_TYPE" => "application/json" }
      delete "/logout", '{"auth_token":"'+user.authtokens.last.token+'sadklsa'+'"}', headers
